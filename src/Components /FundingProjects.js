...
const FundingProjects = () => {
  ...
  const tableRows = projectsData.map((project) => (
    <tr key={project.sno}>
      <td>{project.sno}</td>
      <td>{project.title}</td>
      <td style={{ color: project.agency === 'Active' ? 'green' : 'red' }}>{project.agency}</td>
    </tr>
  ));
  return (
    <>
      <Header />
      <div className="MOUProjects">
        <div className="MOUProjectsHeading">Existing MoUs</div>
        <div className="MOUProjectsTable">
          <table>
            <thead>
              <tr>
                <th>S.No</th>
                <th>Name of the MOUs</th>
                <th>Status</th>
              </tr>
            </thead>
